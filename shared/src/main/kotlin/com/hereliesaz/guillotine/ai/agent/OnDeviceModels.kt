...
data class OnDeviceModel(
    ...
    /**
     * Relative capability inside its own category (1 = basic/lightweight, 8 = strongest workstation
     * tier). Used only after device-fit checks; it never overrides a RAM/storage incompatibility.
     */
...
val RECOMMENDED_ON_DEVICE_MODELS: List<OnDeviceModel> = listOf(
    ...
    OnDeviceModel(
        ...
        abilities = "Fast Qwen planner below 0.5 GB; retained as a compact tier even after Qwen3.5 arrived.",
        limitations = "Less reliable on long, multi-step plans than the newer/larger choices.",
        capabilityTier = 3,
    ),
    OnDeviceModel(
        id = "gemma3-1b-int4",
        label = "Gemma 3 1B Instruct (int4) — legacy-safe",
        ...
        abilities = "Compact proven .task assistant and compatibility fallback for devices unhappy with newer LiteRT-LM bundles.",
        limitations = "Older generation than Qwen3.5 / Gemma 4; Gemma Terms of Use apply.",
        capabilityTier = 4,
    ),
    OnDeviceModel(
        id = "minicpm5-1b-int4",
        ...
        abilities = "Built for edge assistants, coding agents and tool-use workflows with a modest footprint.",
        limitations = "More startup/memory cost than the sub-0.6B tier.",
        capabilityTier = 4,
    ),
    OnDeviceModel(
        id = "qwen3.5-0.8b-int8",
        label = "Qwen3.5 0.8B (int8) — newest light hybrid",
        fileName = "Qwen3.5-0.8B_int8.litertlm",
        sizeBytes = 963_000_000L,
        license = "Apache-2.0",
        gated = false,
        repoUrl = hfRepo("litert-community/Qwen3.5-0.8B"),
        downloadUrl = hfResolve("litert-community/Qwen3.5-0.8B", "Qwen3.5-0.8B_int8.litertlm"),
        abilities = "Current Qwen3.5 hybrid architecture with near-flat long-context memory growth; mobile GPU path is supported.",
        limitations = "Almost 1 GB despite the 0.8B parameter count; text-only build in Guillotine.",
        capabilityTier = 4,
        verifyBySize = false,
    ),
    OnDeviceModel(
        id = "minicpm5-2b-int4",
        label = "MiniCPM5 2B (int4) — efficient heavy phone",
        ...
        abilities = "High-capability edge model with hybrid reasoning and tool-oriented behavior in a 1.55 GB phone build.",
        limitations = "Requires LiteRT-LM 0.16+ and is best on devices with ample RAM and a working GPU backend.",
        capabilityTier = 5,
        verifyBySize = false,
    ),
    OnDeviceModel(
        id = "qwen3.5-2b-int8",
        label = "Qwen3.5 2B (int8) — strong modern mobile",
        fileName = "Qwen3.5-2B_int8.litertlm",
        sizeBytes = 1_970_000_000L,
        license = "Apache-2.0",
        gated = false,
        repoUrl = hfRepo("litert-community/Qwen3.5-2B"),
        downloadUrl = hfResolve("litert-community/Qwen3.5-2B", "Qwen3.5-2B_int8.litertlm"),
        abilities = "New Qwen3.5 planner with hybrid linear/full attention and strong multi-turn behavior.",
        limitations = "About 2 GB; reserve it for phones/tablets with comfortable RAM headroom.",
        capabilityTier = 6,
        verifyBySize = false,
    ),
    OnDeviceModel(
        id = "qwen3.5-4b-int4",
        label = "Qwen3.5 4B (mixed int4) — flagship mobile",
        fileName = "Qwen3.5-4B_mixed_int4.litertlm",
        sizeBytes = 2_570_000_000L,
        license = "Apache-2.0",
        gated = false,
        repoUrl = hfRepo("litert-community/Qwen3.5-4B"),
        downloadUrl = hfResolve("litert-community/Qwen3.5-4B", "Qwen3.5-4B_mixed_int4.litertlm"),
        abilities = "Newest high-end Qwen mobile tier; mixed-int4 build is specifically aimed at RAM-constrained Android devices.",
        limitations = "2.57 GB. Best on modern high-memory phones/tablets; older devices should use a smaller tier.",
        capabilityTier = 7,
        verifyBySize = false,
    ),
    OnDeviceModel(
        id = "gemma4-e2b",
        label = "Gemma 4 E2B — multimodal-era planner",
        fileName = "gemma-4-E2B-it.litertlm",
        sizeBytes = 2_590_000_000L,
        license = "Apache-2.0",
        gated = false,
        repoUrl = hfRepo("litert-community/gemma-4-E2B-it-litert-lm"),
        downloadUrl = hfResolve("litert-community/gemma-4-E2B-it-litert-lm", "gemma-4-E2B-it.litertlm"),
        abilities = "Current Gemma 4 edge model with native function-calling support and memory-mapped embeddings; strong flagship alternative.",
        limitations = "2.59 GB download. Guillotine currently uses it as a text planner; frame vision/audio remain delegated to specialist tools.",
        capabilityTier = 7,
        verifyBySize = false,
    ),
)
 ...
 * Ollama-hosted local models, which lets desktops scale into larger planners while keeping the editor
 * integration on the same OpenAI-compatible tool-calling protocol.
 *
 * [fileName] stores the local selector as `ollama:<tag>`; desktop Settings treats these as Ollama
 * models rather than files and can pull/select them directly. Sizes are published Ollama bundle sizes
 * used for device-fit/storage guidance.
 */
val RECOMMENDED_DESKTOP_ASSISTANT_MODELS: List<OnDeviceModel> = listOf(
    OnDeviceModel(
        id = "desktop-qwen3.5-0.8b",
        label = "Qwen3.5 0.8B — light desktop",
        fileName = "ollama:qwen3.5:0.8b",
        sizeBytes = 1_000_000_000L,
        license = "Apache-2.0",
        gated = false,
        repoUrl = "https://ollama.com/library/qwen3.5",
        downloadUrl = null,
        abilities = "Newest lightweight Qwen3.5 tier with tools, thinking and multimodal-era architecture.",
        limitations = "Good for simple/medium edits; use a larger desktop tier for long multi-step planning.",
        capabilityTier = 3,
    ),
    OnDeviceModel(
        id = "desktop-qwen3.5-2b",
        label = "Qwen3.5 2B — efficient desktop",
        fileName = "ollama:qwen3.5:2b",
        sizeBytes = 2_700_000_000L,
        license = "Apache-2.0",
        gated = false,
        repoUrl = "https://ollama.com/library/qwen3.5",
        downloadUrl = null,
        abilities = "Current Qwen3.5 agent/tool tier with a moderate local footprint.",
        limitations = "A compromise for laptops; workstation-class models reason more deeply.",
        capabilityTier = 4,
    ),
    OnDeviceModel(
        id = "desktop-phi4-mini",
        label = "Phi-4 Mini 3.8B — compact tool-use alternate",
        ...
        downloadUrl = null,
        abilities = "Compact reasoning model with explicit function-calling support; useful as an alternate planner.",
        limitations = "Older than Qwen3.5/Gemma 4 but still useful as a small tool-calling baseline.",
        capabilityTier = 4,
    ),
    OnDeviceModel(
        id = "desktop-qwen3.5-4b",
        label = "Qwen3.5 4B — balanced desktop",
        fileName = "ollama:qwen3.5:4b",
        sizeBytes = 3_400_000_000L,
        license = "Apache-2.0",
        gated = false,
        repoUrl = "https://ollama.com/library/qwen3.5",
        downloadUrl = null,
        abilities = "Strong current-generation planner with tools/thinking and a reasonable laptop/workstation footprint.",
        limitations = "Benefits from acceleration; lower tiers leave more RAM for video editing.",
        capabilityTier = 5,
    ),
    OnDeviceModel(
        id = "desktop-qwen3.5-9b",
        label = "Qwen3.5 9B — strong desktop",
        fileName = "ollama:qwen3.5:9b",
        sizeBytes = 6_600_000_000L,
        license = "Apache-2.0",
        gated = false,
        repoUrl = "https://ollama.com/library/qwen3.5",
        downloadUrl = null,
        abilities = "Strong local agent/planner tier with a 256K context family and current Qwen3.5 architecture.",
        limitations = "6.6 GB model; best on 24–32 GB+ systems or machines with useful GPU acceleration.",
        capabilityTier = 6,
    ),
    OnDeviceModel(
        id = "desktop-gemma4-12b",
        label = "Gemma 4 12B — multimodal workstation",
        fileName = "ollama:gemma4:12b",
        sizeBytes = 7_600_000_000L,
        license = "Apache-2.0",
        gated = false,
        repoUrl = "https://ollama.com/library/gemma4",
        downloadUrl = null,
        abilities = "Newest Gemma workstation tier with tools, thinking and multimodal understanding; strong local planner.",
        limitations = "7.6 GB model and meaningful working memory; avoid on small laptops while editing video.",
        capabilityTier = 7,
    ),
    OnDeviceModel(
    ...
    OnDeviceModel(
        id = "desktop-qwen3.5-27b",
        label = "Qwen3.5 27B — high-memory workstation",
        fileName = "ollama:qwen3.5:27b",
        sizeBytes = 17_000_000_000L,
        license = "Apache-2.0",
        gated = false,
        repoUrl = "https://ollama.com/library/qwen3.5",
        downloadUrl = null,
        abilities = "Very strong current-generation local planning tier for substantial RAM/VRAM workstations.",
        limitations = "17 GB model; overkill for mainstream desktops and deliberately not recommended unless memory headroom is large.",
        capabilityTier = 8,
    ),
)
