...
fun TutorialDialog(onDismiss: () -> Unit) {
    val steps = remember { parseSections(readDoc("TUTORIAL.md")) }
...
fun FaqDialog(onDismiss: () -> Unit) {
    val entries = remember { parseSections(readDoc("FAQ.md")) }
...
private fun readDoc(fileName: String): String =
    runCatching { Thread.currentThread().contextClassLoader?.getResourceAsStream("help/$fileName")?.bufferedReader()?.use { it.readText() } ?: "" }
        .getOrDefault("")
